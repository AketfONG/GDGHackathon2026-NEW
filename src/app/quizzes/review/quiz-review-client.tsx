...
import { getUiQuizById } from "@/lib/ui-quizzes";
...
type Props = {
  /** Preset UI-only quiz id when app is in Demo mode; null in Live mode. */
  fallbackDemoQuizId: string | null;
};

export function QuizReviewPageClient({ fallbackDemoQuizId }: Props) {
  const searchParams = useSearchParams();
  const quizId = searchParams.get("quizId") ?? fallbackDemoQuizId ?? undefined;
      ...
      <div className="min-h-screen">
        ...
        <main className="mx-auto w-full max-w-6xl px-4 py-8">
          <p className="rounded border border-slate-200 bg-white p-4 text-slate-700">
            {fallbackDemoQuizId === null && !searchParams.get("quizId")
              ? "Quiz review for sample presets is available in Demo mode on the dashboard, or open this page with ?quizId=your-quiz-id."
              : "Quiz not found."}
          </p>
        </main>
      </div>
